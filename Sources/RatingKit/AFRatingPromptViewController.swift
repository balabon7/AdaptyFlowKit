// AFRatingPromptViewController.swift
// AppSceneKit
//
// Pre-prompt with stars before Apple's SKStoreReviewController.
// 4-5 stars → Apple prompt. 1-3 stars → feedback URL.

import UIKit

// MARK: - RatingPromptViewController

final class AFRatingPromptViewController: UIViewController {

    // MARK: - Properties

    private let configuration: AFRatingKitConfiguration
    private let accentColor: UIColor
    private let onResult: (AFRatingResult) -> Void
    private var isAnswered = false

    // MARK: - Init

    init(configuration: AFRatingKitConfiguration, accentColor: UIColor = .systemBlue, onResult: @escaping (AFRatingResult) -> Void) {
    ...
    private lazy var starsView: RatingStarsView = {
        let v = RatingStarsView(stars: 5, initial: 5)
    ...
    private lazy var submitButton: UIButton = {
        ...
        b.backgroundColor = AFAppearance.ratingSubmitButtonColor ?? accentColor
        b.layer.cornerRadius = 12
    ...
    override func viewDidLoad() {
        ...
        titleLabel.text = "Rate \(configuration.appName)"
        buildUI()
        layoutUI()
...
/// Row of 5 stars. Yellow color, bounce animation, haptic on each tap.
final class RatingStarsView: UIStackView {
    ...
    // Same yellow as in RateAppViewController
